...
for t = 0:size(precios,1)-npm
    ...
    % Vector de tamaño dinámico de promedios moviles pm
    pm(npm+t) = mean(precios(t+1:npm+t)); % Computar promedio de los npm dais anteriores
    
    % Generar señales de compra y venta comparando el precio de cierre
    % con el del promedio movil calculado anteriormente
    if pm(npm+t) < precios(npm+t)
        
        % Señal de compra, i.e. si el promedio movil es menor que 
        % el precio actual, es decir, el precio cruza hacia arriba el promedio.
        
        % Obtener el número de acciones adquiribles con el capital actual
        % tomando en cuenta la comisión de transacción por unidad
    ...
    % Actualizar el número de acciones en poseción 
    nac(npm+t+1) = nac(npm+t) + units;
    
    % Calcular el capital disponible para el siguiente trade
    cap(npm+t+1) = cap(npm+t) - precios(npm+t) * units-com*precios(npm+t)*abs(units);
end

% Visualizacion de los resultados, el capital y el numero de acciones
% se ven como señales complementarias ya que en cada trade se está
% utilzando todo el capital disponible 
T = (1:size(precios,1))';
figure(1);
subplot(4,1,1);
